...
export default function ScrapbookViewer() {
 ...
 const [scrapbookData, setScrapbookData] = useState<ScrapbookData | null>(null);
 ...
 const fetchScrapbookData = async (id: string) => {
   ...
   setError(null);
   try {
     const response = await fetch(`https://scrapbook.hackclub.com/api/users/${id}`);
     if (!response.ok) {
 ...
 if (error) {
   return (
     <View style={styles.centerContainer}>
       ...
       <Button title="Try Again" onPress={() => storedUsername && fetchScrapbookData(storedUsername)} color="#ec3750" />
     </View>
   );
 }
       ...
       <View style={styles.profileCard}>
         <View style={styles.profileHeader}>
           {scrapbookData.profile.avatar && (
             <Image source={{ uri: scrapbookData.profile.avatar }} style={styles.avatar} />
           )}
           <View style={styles.profileInfo}>
             <Text style={styles.username}>{scrapbookData.profile.username}</Text>
             <Text style={styles.streakInfo}>
               Current Streak: {scrapbookData.profile.streakCount} • Max Streak: {scrapbookData.profile.maxStreaks}
             </Text>
           </View>
         </View>
         <View style={styles.profileLinks}>
           {scrapbookData.profile.website && (
             <TouchableOpacity style={styles.linkButton} onPress={() => handleLinkPress(scrapbookData.profile.website!)}>
               <Text style={styles.linkText}>Website</Text>
             </TouchableOpacity>
           )}
           {scrapbookData.profile.github && (
             <TouchableOpacity style={styles.linkButton} onPress={() => handleLinkPress(scrapbookData.profile.github!)}>
               <Text style={styles.linkText}>GitHub</Text>
             </TouchableOpacity>
           )}
         </View>
       </View>
       <Text style={styles.sectionTitle}>Posts ({scrapbookData.posts.length})</Text>
       {scrapbookData.posts.map(renderPost)}
       <View style={styles.headerContainer}>
         <Text style={styles.slackIdText}>Username: {storedUsername}</Text>
         <Button title="Change Username" onPress={handleChangeId} color="#ec3750" />
       </View>
...
const styles = StyleSheet.create({
 ...
 profileCard: { backgroundColor: "#1E1E1E", padding: 20, marginBottom: 16, borderRadius: 16, borderWidth: 1, borderColor: "#333333" },
 ...
 linkButton: { backgroundColor: "#ec3750", paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8 },
 ...
 postText: { fontSize: 16, color: "#FFFFFF", lineHeight: 24, marginBottom: 12 },
 attachmentsContainer: { marginBottom: 12 },
 attachmentImage: { width: width - 64, height: 200, borderRadius: 8, marginBottom: 8 },
 ...
 inputContainer: { width: "85%", backgroundColor: "#1E1E1E", borderRadius: 16, padding: 20, borderWidth: 1, borderColor: "#333333" },
 inputLabel: { color: "#FFFFFF", marginBottom: 12, fontSize: 16, fontWeight: "600" },
 textInput: { backgroundColor: "#23272f", color: "#FFFFFF", paddingVertical: 12, paddingHorizontal: 14, borderRadius: 8, marginBottom: 14, fontSize: 16, borderWidth: 1, borderColor: "#333333" },
